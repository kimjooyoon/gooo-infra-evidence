...
The Actions workflow verifies the public OpenTofu `v1.12.6` release and its
Linux archive digest. It does not execute the downloaded OpenTofu binary.
...
The mapping ontology is Gooo-owned and requires exact typed identities:
...
- proofs: `FOUNDATION 4`, `COHERENCE 5`, `REGRESSION 3`;
...
lock. A normal bundle contains five exact relations: one resource-to-
...
Build and test executions are explicitly zero in the workflow. Their wall
time, peak RSS, and cache fields are `null` with `UNKNOWN` / `NOT_EXECUTED`,
not fabricated zero timings. Go/Gooo lines, files, and directories exclude
root `README.md` and are captured as exact Actions evidence. The workflow does
not run `go build`, `go test`, `gofmt`, `go vet`, `actionlint`, or any `tofu`
command. It does not apply infrastructure, install providers, write remote
state, deploy, or contact a service runtime. Any future accidental execution
