...
if test "$#" -ne 8; then
  echo "usage: evaluate-v2.sh ROOT CANDIDATE RUNTIME GRAPH SYMBOL_RECEIPT OUTPUT HEAD_SHA PHASE" >&2
  exit 64
fi
...
for file in "$denominator" "$core_lock" "$project" "$runtime" "$graph" "$symbol_receipt"; do
...
test -s "$candidate/$terraform_declaration" && declaration_available=true || declaration_available=false
if test "$declaration_available" = true && grep -Fq "$(jq -r .artifact_digest "$project")" "$candidate/$terraform_declaration" && grep -Fq "$(jq -r .deployment_target "$project")" "$candidate/$terraform_declaration"; then
...
jq -S -n \
  ...
  --slurpfile runtime "$runtime" --slurpfile graph "$graph" --slurpfile symbol "$symbol_receipt" \
  ...
  --argjson declaration_available "$declaration_available" --argjson declaration_matches "$declaration_matches" \
  ...
  def domain($cell):
    ...
    elif $cell.id=="TERRAFORM_DECLARATION" then if $declaration_matches then direct_closed($cell.closed_reason) elif ($declaration_available|not) then direct_unknown($cell.unknown_reason;$cell.next_operation) else direct_refuted("TERRAFORM_DECLARATION_BINDING_MISMATCH";"RESTORE_TERRAFORM_DECLARATION") end
  ...
  ([($declaration_matches and plan_ok),(plan_ok and state_ok),symbol_ok,provenance_ok,deployment_ok,runtime_ok]|map(select(.==true))|length) as $semantic_edges |
  {
    ...
    evidence_digests:{terraform_declaration:$terraform_declaration_digest,terraform_plan:$terraform_plan_digest,terraform_state:$terraform_state_digest,openapi:$openapi_digest,
    ...
    authority:{binding:"RELEASED_GOOO_ACTIVITY_RESOLUTION_RECEIPTS",core_activity_receipts:$core_receipts,core_activity_total:12,core_identity_anchors:([$d.cells[]|select(.core_identity_anchor==true)]|length),
      service_symbol_parser:"GO_AST",terraform_execution:"NOT_CLAIMED",terraform_validation:"NOT_CLAIMED",live_cloud_state:"NOT_CLAIMED",deployment_execution:"NOT_CLAIMED",live_network_probe:"NOT_CLAIMED"},
