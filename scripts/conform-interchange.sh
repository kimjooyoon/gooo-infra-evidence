...
if jq -e '.schema=="gooo/interchange/project/v1" and .domain=="infra-evidence" and (.project_id|type)=="string" and (.release.target_commit_sha|test("^[0-9a-f]{40}$")) and (.release.source_asset_sha256|test("^[0-9a-f]{64}$")) and .relation_count==1 and (.unknown_count==0 or .unknown_count==1)' "$bundle/project.json" >/dev/null 2>&1 &&
...
jq -S -n --argjson checks "$checks" --slurpfile project "$bundle/project.json" --slurpfile conformance "$bundle/conformance.json" '
  ...
  {schema:"gooo/infra-evidence/interchange-conformance/v1",fixture_id:$project[0].fixture_id,domain:$project[0].domain,
